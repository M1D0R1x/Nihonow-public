...
from .models import UserProgress, DailyWord, Kanji, QuizQuestion
...
# Categorize the kanji for n5_kanji
n5_number_order = ['一', 'ニ', '三', '四', '五', '六', '七', '八', '九', '十', '百', '千', '万']
kanji_numbers = [k for k in n5_kanji_data if k['character'] in n5_number_order]
kanji_numbers = sorted(kanji_numbers, key=lambda k: n5_number_order.index(k['character']))
kanji_environment = [k for k in n5_kanji_data if
                     k['character'] in ['雨', '火', '花', '山', '川', '空', '月', '気', '魚', '金', '木', '水', '土',
                                        '日', '電', '天']]
kanji_people = [k for k in n5_kanji_data if
                k['character'] in ['人', '子', '女', '男', '父', '母', '友', '手', '足', '耳', '目', '口', '名', '生']]
kanji_directions = [k for k in n5_kanji_data if
                    k['character'] in ['上', '下', '左', '右', '中', '外', '前', '後', '西', '東', '南', '北']]
kanji_verbs = [k for k in n5_kanji_data if
               k['character'] in ['飲', '会', '出', '見', '言', '休', '行', '書', '食', '入', '買', '聞', '読', '来',
                                  '立', '話', '学', '分']]
kanji_adjectives = [k for k in n5_kanji_data if
                    k['character'] in ['安', '多', '大', '小', '少', '新', '古', '高', '長', '白', '半']]
kanji_misc = [k for k in n5_kanji_data if k['character'] not in (
        [k['character'] for k in kanji_numbers] + [k['character'] for k in kanji_environment] +
        [k['character'] for k in kanji_people] + [k['character'] for k in kanji_directions] +
        [k['character'] for k in kanji_verbs] + [k['character'] for k in kanji_adjectives]
)]


def n5_kanji(request):
    context = {
        'numbers': kanji_numbers,
        'environment': kanji_environment,
        'people': kanji_people,
        'directions': kanji_directions,
        'verbs': kanji_verbs,
        'adjectives': kanji_adjectives,
        'misc': kanji_misc,
    }
    ...
    try:
        ...
        if profile.email_confirmed:
            ...
            message = (
                    ...
                    'If not redirected, <a href="' + reverse(
                'login') + '" class="text-primary">click here to go to the login page</a>.'
            )
            ...
            try:
                ...
                if hasattr(user, 'profile') and not user.profile.email_confirmed:
                    messages.error(request,
                                   "Please confirm your email to log in. Check your inbox for the confirmation link.")
                else:
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if user.is_active:
                ...
                if hasattr(profile, 'last_confirmation_sent') and (
                        timezone.now() - profile.last_confirmation_sent).total_seconds() < 300:  # 300 seconds = 5 minutes
                    ...
                    messages.success(request,
                                     "A verification link has been sent. Please check your inbox and spam folder. If not received in 5 minutes, resend again.")
        except User.DoesNotExist:
    ...
    if request.method == "POST":
        ...
        if 'dailyword_csv' in request.FILES:
            ...
            if not required_headers.issubset(reader.fieldnames):
                messages.error(request,
                               "DailyWord CSV must have headers: word, reading, english_meaning (date optional)")
                return redirect('admin_bulk_upload')

            current_date = timezone.now().date()
            for i, row in enumerate(reader):
                try:
                    date = datetime.strptime(row["date"], "%Y-%m-%d").date() if has_date else current_date + timedelta(
                        days=i)
        ...
        # Handle Kanji upload
            ...
            if not required_headers.issubset(reader.fieldnames):
                messages.error(request,
                               "Kanji CSV must have headers: character, level, on_reading, kun_reading, meaning, stroke_count")
                return redirect('admin_bulk_upload')
        ...
        # Handle QuizQuestion upload
            ...
            decoded_file = csv_file.read().decode("utf-8").splitlines()
            reader = csv.DictReader(decoded_file)
            required_headers = {"level", "question", "wrong_answer1", "wrong_answer2", "wrong_answer3",
                                "correct_answer"}
            if not required_headers.issubset(reader.fieldnames):
                messages.error(request,
                               "Quiz CSV must have headers: level, question, wrong_answer1, wrong_answer2, wrong_answer3, correct_answer")
                return redirect('admin_bulk_upload')
